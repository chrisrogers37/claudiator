...
Compare managed files between the claudosseum repo and `~/.claude/`, then interactively sync differences in either direction.
...
```
~/.local/share/claudosseum/backups/<YYYY-MM-DD_HHMMSS>/
```
...
Read `~/.claude/.claudosseum-repo` to get the repo path. If the file doesn't exist, tell the user: "Run `/claudosseum-setup` from the claudosseum repo, or run `./install.sh`" and stop.
...
═══════════════════════════════════════════
  ...
  commands/claudosseum-sync.md  ⚡ modified
...
Follow the same two-sub-step procedure as Step 4.5 in `/claudosseum-setup`:
...
Follow the same procedure as Step 4.6 in `/claudosseum-setup`:
...
Follow the same procedure as Step 4.7 in `/claudosseum-setup`:
...
Your previous files were backed up to:
  ~/.local/share/claudosseum/backups/<timestamp>/

To restore any file, copy it back:
  cp ~/.local/share/claudosseum/backups/<timestamp>/commands/my-file.md ~/.claude/commands/
═══════════════════════════════════════════
```
